...
Enable the agent with env vars: `GRAPH_TENANT_ID`, `GRAPH_CLIENT_ID`, `GRAPH_CLIENT_SECRET`, `GRAPH_MAILBOX` (+ optional `ACH_FROM`, `ACH_SUBJECT`, `POLL_INTERVAL_MS`). **The parser exists in two places** (`index.html` and `server.js`) — keep them in sync. Server-side payments currently persist to `data/payments.json` (gitignored, and **ephemeral on Railway** — another reason the Postgres migration matters).
